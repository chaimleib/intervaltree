...
class IntervalTree(collections.MutableSet):
    ...
    def search(self, begin, end=None, data=None, strict=False):
        ...
        If data is not None, filter result to intervals where
        interval.data in data is True. When data is specified,
        Intervals without data are returned if and only None in data.
        ...
        if end is None:
            try:
                ...
                if not isinstance(data, collections.Iterable):
                    # TODO: How to handle bad input? Ignoring for now
                    return result
                result = set(
                    iv for iv in result
                    if iv.data in data
